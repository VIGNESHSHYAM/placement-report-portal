...
// Dynamically import the component to improve load time
...
const UserLogin = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await loginUser({ registrationNumber, password });

      if (data.result) {
        localStorage.setItem("userToken", data.result.data.token);
